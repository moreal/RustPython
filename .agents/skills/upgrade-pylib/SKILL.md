...
- Run `quick` with its default automatic commit enabled. Do not pass `--no-commit`
  solely to add agent attribution to the mechanical CPython sync commit.
- Keep the automatic sync commit authored only by `CPython Developers` with the
  message generated by `update_lib`; do not amend it for agent attribution.
- Put any agent-authored restorations, fixes, or manually selected test markers in
  a separate follow-up commit and disclose that assistance according to the active
  contribution policy.
